...
sys.path.append('../')
from utility import get_center_of_boxes, get_bbox_width 
...
class Tracker :
    ...
    def get_obj_tracker(self , frames , read_from_stub = False , stub_path = None    )  :
        ...
        for frame_num , detection in enumerate(detections) :
            ...
            # tracer 
            detection_with_tracker = self.track.update_with_detections(detection_supervision)
    ...
    def draw_ellipse(self, frame, bbox, color, track_id = None ) :
        ...
        cv2.ellipse(frame , center=(x_center,y2),
                    ...
                    angle=0.0, startAngle= -60 , endAngle= 230, 
        ...
        if track_id is not None :
            ...
            if track_id > 99  :
                x1_text -= 8 
            cv2.putText(frame, f"{track_id}", (int(x1_text),int(y1_rect+10)), cv2.FONT_HERSHEY_SIMPLEX , 0.6,(0,0,0),2 )



        return frame 
    
    def draw_tri(self, frame, bbox, color ) :
        y = int(bbox[1])
        ...
        for frame_num , frame in enumerate(video_frames) :
            ...
            for track_id , player in player_dict.items() :
                ...
                if player.get('has_ball' , False)  : 
                    frame = self.draw_tri(frame, player["bbox"] , (0,0,255))


            for _, referee in referee_dict.items() :
                frame = self.draw_ellipse(frame , referee["bbox"] , (52, 76, 235) , None )

            for track_id, ball in ball_dict.items() :
                frame = self.draw_tri(frame, ball["bbox"], (52, 76, 235) )
